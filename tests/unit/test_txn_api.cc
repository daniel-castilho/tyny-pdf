...
static void test_txn_create_free(void) {
  // Need a mock document - for now test with nullptr
  pc_txn* txn = nullptr;
  pc_status s = pc_txn_create(nullptr, &txn);
  // Should fail with null document
  ASSERT_STATUS_EQ(s, PC_ERR_ARGUMENT);
  ASSERT_INT_EQ((size_t)txn, 0);
}

int main(void) {
  test_txn_create_free();
