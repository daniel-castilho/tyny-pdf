...
| `actions/checkout`, `actions/setup-python` | v7.0.1 `3d3c42e5...` and v7.0.0 `5fda3b95...` (node24-targeting; the v4.4.0/v5.6.0 pins of PR #2 were node20, deprecated 2025-09-19), full commit shas with the version as a comment (E-2, closed) | MIT / BSD-3-Clause | none | runner plumbing; writing our own checkout is not a saving | Low | present at `.github/workflows/gates.yml` (both) and `osv-scan.yml` (checkout only) |
...
| `osv-scanner` | `google/osv-scanner-action` v2.6.0, commit `a345acffa...` pinned in `.github/workflows/osv-scan.yml` (PR #57; a docker action, so no Node runtime) | Apache-2.0 | none | advisories have to be looked up by someone, and re-implementing a vulnerability feed is not our edge | Low - the SBOM stays valid and the job can be swapped | present since PR #57, and deliberately **not** in the PR gate, so an unrelated advisory cannot block a documentation fix |
...
| E-5 | `docs/security/advisories.md` is referenced by ADR-0004 section 4 and is not in the tree | **Closed**: the file is in the tree since PR #18 (`054c7b1`), and the scheduled `osv-scanner` job that updates it arrived with PR #57 (`87f58ed`) |
