...
static void json_write_key_value_str(char* buf, size_t* pos, size_t cap, int indent, const char* key, const char* value) {
...
static void json_write_key_value_int(char* buf, size_t* pos, size_t cap, int indent, const char* key, int64_t value) {
  for (int i = 0; i < indent; ++i) json_append_str(buf, pos, cap, "  ");
  json_append_escaped(buf, pos, cap, key);
  json_append_str(buf, pos, cap, ": ");
...
static int pc_sidecar_to_canonical_json(const pc_sidecar* sidecar, char* buf, size_t cap) {
  if (!sidecar || !buf || cap < 256) return -1;
  size_t pos = 0;

  // Root object - keys in alphabetical order: annotations, document, format_version, is_stale, stale_reason, unknown
  json_append_str(buf, &pos, cap, "{\n");
  ...
  json_write_key_value_str(buf, &pos, cap, 2, "path", sidecar->document_path ? sidecar->document_path : "");
  json_write_key_value_str(buf, &pos, cap, 2, "sha256", sidecar->document_sha256 ? sidecar->document_sha256 : "");
...
pc_status pc_sidecar_write(const char* doc_path, const pc_sidecar* sidecar) {
  ...
  // Check excluded keys (R6)
  const char* excluded_keys[] = {
...
pc_status pc_sidecar_read(const char* doc_path, pc_sidecar** out_sidecar) {
  ...
  if (!s->document_sha256) s->document_sha256 = strdup("");
  ...
  s->annotations_json = extract_json_string(content, "annotations");
  if (!s->annotations_json) s->annotations_json = strdup("[]");

  s->unknown_json = extract_json_string(content, "unknown");
  if (!s->unknown_json) s->unknown_json = strdup("{}");
...
pc_status pc_sidecar_try_lock(const char* doc_path) {
  ...
  int len = snprintf(lock_content, sizeof(lock_content), "pid=%d time=%ld\n", getpid(), time(nullptr));
...
pc_status pc_sidecar_validate_annotation_id(const char* annotation_id) {
  ...
  const char* base32_lower = "abcdefghijkmnopqrstuvwxyz234567";
