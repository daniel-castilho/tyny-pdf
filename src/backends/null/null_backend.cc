...
static pc_status null_doc_open(const char *path, const char *password, void **out_backend_doc) {
    ...
    if (!doc) return { sizeof(pc_status), PC_ERR_MEMORY, 0, "OOM" };
    doc->page_count = 5;
    *out_backend_doc = doc;
...
static pc_status null_page_get(void *backend_doc, uint32_t index, void **out_backend_page) {
    ...
    if (!page) return { sizeof(pc_status), PC_ERR_MEMORY, 0, "OOM" };
    page->index = index;
    *out_backend_page = page;
    return { sizeof(pc_status), PC_ERR_NONE, 0, nullptr };
}

static pc_status null_page_render(void *backend_page, const pc_render_params *params, pc_pixmap *out_pixmap) {
    (void)backend_page; (void)params;
    ...
    if (!data) return { sizeof(pc_status), PC_ERR_MEMORY, 0, "OOM" };
