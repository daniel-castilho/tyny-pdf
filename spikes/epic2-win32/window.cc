...
static HRESULT CreateD2DFactory(WindowData* data) {
  ...
  HRESULT hr = D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory3),
                                 &options, reinterpret_cast<void**>(&data->d2d_factory));
  return hr;
}
...
static HRESULT CreateD2DDevice(WindowData* data) {
  ...
  hr = data->d2d_factory->CreateDevice(data->d2d_device.Get(), &data->d2d_device);
  if (FAILED(hr))
    return hr;
...
static void ResizeSwapchain(WindowData* data) {
  ...
  HRESULT hr =
      data->swapchain->ResizeBuffers(0, data->width, data->height, DXGI_FORMAT_B8G8R8A8_UNORM, 0);
  if (SUCCEEDED(hr)) {
...
static void RenderFrame(WindowData* data) {
  ...
  D2D1_RECT_F rect =
      D2D1::RectF(0, 0, static_cast<float>(data->width), static_cast<float>(data->height));
  data->d2d_context->FillRectangle(
      &rect, data->d2d_context->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::LightGray)));
...
pc_status tynypdf_ui_run(const char* pdf_path, const pc_backend_api* backend, uint32_t page) {
  ...
  HWND hwnd = CreateWindowExW(0, kWindowClassName, L"Tyny PDF", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
